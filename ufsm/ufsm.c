                                             ...
                                             void* event_context)
{
                                                ...
                                                t; t = t->next)
    {
        ...
        {
            err = ufsm_make_transition(m, t, s->parent_region, event_context);
            break;
        }
    }
                                      ...
                                      void* event_context)
{
    ...
    for (struct ufsm_entry_exit *e = s->entry; e; e = e->next)
    {
        ...
        e->f(m->context, s->context, event_context);
    }
    ...
    for (struct ufsm_doact *d = s->doact; d; d = d->next)
    {
        ...
        d->f_start(m->context, s->context, event_context,&ufsm_completion_handler);
    }
                                    ...
                                    void* event_context)
{
    ...
    for (struct ufsm_doact *d = s->doact; d; d = d->next)
        d->f_stop(m->context, s->context, event_context);
                                    ...
                                    void* event_context)
{
    ...
    {
        bool guard_result = g->f(m->context, t->source->context, event_context);
                                              ...
                                              void* event_context)
{
    ...
    for (uint32_t i = 0; i < c; i++)
    {
        ...
        if (ps)
        {
            ...
            if (pr != ancestor)
                ufsm_enter_state(m, ps, event_context);
        }
    }
                                              ...
                                              void* event_context)
{
    ...
    *lca = ancestor;
    ...
    while (states_to_leave)
    {
        ...
        if (rl->parent_state)
        {
            ufsm_leave_state(m, rl->parent_state, event_context);
            rl->current = NULL;
                                              ...
                                              void* event_context)
{
    ...
    for (uint32_t i = 0; i < c; i++)
    {
        ...
        ufsm_leave_state(m, s2, event_context);
                                              ...
                                              void* event_context)
{
    ...
    if (regions->has_history && regions->history)
    {
        ...
        ufsm_enter_state(m, regions->current, event_context);
        err = UFSM_OK;
    }
                                          ...
                                          void* event_context)
{
    ...
    for (struct ufsm_region *s_r = r; s_r; s_r = s_r->next)
    {
        ...
        if (init_t == NULL) {
            err = ufsm_init_region_history(m, s_r, event_context);
        } else {
                                              ...
                                              void* event_context)
{
    ...
    if (super_exit && parent_state)
    {
        ...
        for (struct ufsm_region *ar = parent_region; ar; ar = ar->next)
            ufsm_leave_state(m, ar->current, event_context);
                                         ...
                                         void* event_context)
{
    ...
    for (struct ufsm_transition *dt = transitions; dt; dt = dt->next)
    {
        if (dt->source == dest)
        {
            if (dt->guard)
            {
                if (ufsm_test_guards(m, dt, event_context))
                {
                                          ...
                                          void* event_context)
{
    ...
    while (transition_count && (err == UFSM_OK))
    {
        ...
        if (!ufsm_test_guards(m, act_t, event_context))
        {
        ...
        if (t->kind == UFSM_TRANSITION_EXTERNAL)
        {
            ...
            ufsm_leave_nested_states(m, src, event_context);
            ufsm_leave_state(m, act_t->source, event_context);

            /* For compound transitions parents must be exited and entered
             * in the correct order.
             * */
            err = ufsm_leave_parent_states(m, src, dest, &lca_region,
                                                             &act_region, event_context);
            if (err != UFSM_OK)
                break;
        }

        ufsm_execute_actions(m, act_t, event_context);

        if ((t->kind == UFSM_TRANSITION_EXTERNAL) &&
            (src->parent_region != dest->parent_region))
        {
            err = ufsm_enter_parent_states(m, lca_region, dest->parent_region, event_context);
        ...
        switch(dest->kind)
        {
            ...
            case UFSM_STATE_SIMPLE:
                ...
                if (t->kind == UFSM_TRANSITION_EXTERNAL)
                {
                    ufsm_enter_state(m, dest, event_context);
                    err = ufsm_process_regions(m, dest, &transition_count, event_context);
                }
            break;
            ...
            case UFSM_STATE_FINAL:
                ...
                err = ufsm_process_final_state(m, act_region, dest,
                                                        &transition_count, event_context);
            break;
...
static ufsm_status_t ufsm_process_completion_events(struct ufsm_machine *m, void* event_context)
{
    ufsm_status_t err = UFSM_OK;
    ...
    {
        err = ufsm_process_completion(m, completed_state, event_context);
        if (err != UFSM_OK)
            return err;
    }
    return err;
...
ufsm_status_t ufsm_init_machine(struct ufsm_machine *m)
{
    ...
    for (struct ufsm_region *r = m->region; r; r = r->next)
    {
        ...
        err = ufsm_make_transition(m, rt, r, NULL);
    ...
    if (err == UFSM_OK)
        err = ufsm_process_completion_events(m, NULL);
                            ...
                            int32_t ev, void* event_context)
{
    ...
    for (struct ufsm_transition *t = r->transition; t; t = t->next)
    {
        ...
        {
            uint32_t e = ufsm_make_transition(m, t, r, event_context);
...
ufsm_status_t ufsm_process (struct ufsm_machine *m, int32_t ev, struct ufsm_context* event_context)
{
    ...
    ufsm_process_completion_events(m, event_context);
    ...
    for (uint32_t i = 0; i < region_count; i++)
    {
        ...
        if (region->current == s)
        {
            if (ufsm_transition (m, region, ev, event_context))
                event_consumed = true;
        }
    }
...
ufsm_status_t ufsm_process_auto(struct ufsm_machine *m, int32_t ev, struct ufsm_context* event_context)
{
	...
	err = ufsm_process(m, ev, event_context);
	...
	if (m->stack.pos != 0)
	{
		if (m->stack.pos == UFSM_STACK_SIZE)
			...
			return UFSM_ERROR_STACK_NOT_ZERO;
	}
	...
	{
		err = ufsm_process(m, q_ev, event_context);
